...
use edger_bevy_util::prelude::math::in_range;
...
pub fn in_range_with_margin(v: f32, range: (f32, f32), margin: f32) -> f32 {
    in_range(v, (range.0 + margin, range.1 + margin))
}
...
impl GridData {
    pub fn cals_fixed_cells_by_side(
        ...
        let cell_size_with_margin =
            in_range(content_size / total as f32, (cell_size_range.0 + margin, cell_size_range.1 + margin));
        let mut cell_count = (content_size / cell_size_with_margin).floor() as usize;
        if cell_count == 0 {
            cell_count = 1;
        }
        let cell_size =
            in_range(content_size / cell_count as f32 - margin, cell_size_range);
